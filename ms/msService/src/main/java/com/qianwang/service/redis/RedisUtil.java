...
public class RedisUtil {
     ...
     * @param map
     */
    public <T> void setMap(String key ,Map<String,T> map,Integer expireTime){
        Jedis jedis = null;
        try {
            jedis = pool.getResource();
     ...
     * 慎用
     *
     * @param key 判断KEY是否存在。不存。则存放一个
     * @return
     */
    public boolean keyExistSet(String key){
        Jedis jedis = null;
        try {
            jedis = pool.getResource();
            boolean has = jedis.exists(key);
            if (!has){
                set(key, "1", 30);
            }
            return has;
    ...
    public static int calTime(int time){
        int prefixTime =time * 60 * 60;
        System.out.println(prefixTime);
        long current=System.currentTimeMillis();//当前时间毫秒数
        long zero=current/(1000*3600*24)*(1000*3600*24)- TimeZone.getDefault().getRawOffset();//今天零点零分零秒的毫秒数
        long twelve=zero+24*60*60*1000-1;//今天23点59分59秒的毫秒数
        long leftTime=(twelve-current)/1000;//今天剩余的秒数
        return (int)(leftTime+prefixTime);
    }

    public static void main(String[] args) {
        System.out.println(calTime(5));
    }
