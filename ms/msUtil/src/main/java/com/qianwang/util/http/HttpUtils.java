package com.qianwang.util.http;
...
public class HttpUtils {
	...
	public static <T> T get(String url,Map headMap,Class<T> clazz) throws Exception {
		...
		try {
			...
			HttpEntity entity = response.getEntity();
			String body = EntityUtils.toString(entity);
			log.info("HttpUtils-get result = {}", body);
			if (entity != null) {
				entity.consumeContent();
			}
	...
	public static String get(String url, Map paramMap) throws Exception {
		CloseableHttpClient client = HttpClientBuilder.create().build();
		...
		try {
			...
			if (paramMap != null && paramMap.size() > 0) {
				urlBuffer.append("?");
				Set<String> headerSet = paramMap.keySet();
				for (String key : headerSet) {
					urlBuffer.append(key);
					urlBuffer.append("=");
					urlBuffer.append(URLEncoder.encode(paramMap.get(key).toString(), "UTF-8"));
					urlBuffer.append("&");
				}
			}
