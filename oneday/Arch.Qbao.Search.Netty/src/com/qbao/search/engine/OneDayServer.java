 ...
 *
 */
public class OneDayServer extends HttpServer {

	private static ESLogger logger = Loggers.getLogger(OneDayServer.class);

	final protected OneDayParser LeixsParser;

	public OneDayServer(boolean shouldRecover) throws Throwable {
		super("LeixsServer", true);
		LeixsParser = new OneDayParser();
	...
	public static void main(String[] args) throws Throwable {
		try {
			...
			System.out.println("+++++ DB.url = " + Config.getBase().get(LoadValues.ONEDAY_CONNECTION).trim());
			...
			new OneDayServer(args.length > 0 && args[0].equals("recover")).start(Properties.port);
