...
echo  `curl "http://10.168.234.231:29010/search?keyword=%E6%9B%B9%E5%AE%89%E8%B7%AF1111&type=recommend"` > monitorsearchapi.html
...
echo `date`  searchApi 搜索参数  ${status_dyonline}  ${Qtime_dyonline} ${numFound_dyonline}
   ...
   if [ "${numFound_dyonline}" == "" ];then
        ...
        echo "`date` 10.168.234.231 solr监控参数为空!  numFound_dyonline=${numFound_dyonline}  "
        content="`date "+%Y-%m-%d_%H:%M:%S"`-10.168.234.231-搜索API监控参数为空!-numFound_dyonline=${numFound_dyonline}"
...
if [ $hh -le 23 ]&&[ $hh -ge 7 ];then
   ...
   if [ ${Qtime_dyonline} -gt 100 ];then
	...
	warn_Qtime_num=`expr $warn_Qtime_num + 1` 
       ...
       content="`date "+%Y-%m-%d_%H:%M:%S"`_10.168.234.231_搜索API返回时间太长_数量${numFound_dyonline}_响应时间${Qtime_dyonline}ms"
