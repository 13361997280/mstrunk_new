...
Kalendae.prototype = {
	...
	setSelected : function (input, draw) {
		...
		i = old_dates.length;
		while(i--) { this.removeSelected(old_dates[i], false, true); }

		i = new_dates.length;
		while(i--) { this.addSelected(new_dates[i], false, true); }
		...
		this.publish('change', this, new_dates);
	},

	addSelected : function (date, draw, notFireChangeEvent) {
		...
		if(!notFireChangeEvent) this.publish('change', this, [date]);
	...
	removeSelected : function (date, draw,notFireChangeEvent) {
		date = moment(date, this.settings.format).hours(12);
		var i = this._sel.length;
		while (i--) {
	...
	draw : function draw() {
		...
		if (opts.directionScrolling) {
			...
			diff = month.diff(diffComparison, 'months', false);
...
Kalendae.Input = function (targetElement, options) {
	...
	util.make('div', {'class' : 'okBtnDiv'}, $container);
	$okButton =  util.make('input', {'type': 'button',value : '确定', 'class' : 'okBtn'}, $container);
	this._events.inputClick = util.addEvent($okButton, 'click', function () {
