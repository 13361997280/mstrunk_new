...
public class DataInputEsService {
	 ...
	 * @return
	 */
	public Integer dealLbsDataInput() {
...
//		lastTime = "2017-08-15 00:21:54";
	 ...
	 */
	public Integer dealQbaoLogDataInput() {
		// 1.ES 定时任务操作日志表里拉取最近一次数据，获取上次操作的最后时间。
		...
		return getJsonObjectStrForConst(lstLong, System.currentTimeMillis(), LoadValues.LABEL_QBAOLOG, LoadValues.LABEL_QBAOLOG, "", "stamp");
	}
	 ...
	 * @return
	 */
	public Integer dealTouTiaoDataInput() {
		// 1.ES 定时任务操作日志表里拉取最近一次数据，获取上次操作的最后时间。
	 ...
	 */
	public Integer getJsonObjectStrForConst(Object fromTime, Object endTime, String indexName, String type,
		...
		try {
			...
			int page = 0;
			int size = 500;
			searchRequestBuilder.setSource(searchSourceBuilder).setFrom((page) * size).setSize(size);
			...
			if (totalNum > size) {
				try {
					boolean flag = true;
					while (flag) {
						searchRequestBuilder.setSource(searchSourceBuilder).setFrom((page + 1) * size).setSize(size);
	...
	public Boolean sendKafka(SearchHit[] hits, String topic) {
		...
		for (int i = 0; i < hits.length; i++) {
			...
			if (topic.equals(LoadValues.LABEL_QBAOLOG)) {
				...
				if (null != qbaoLog) {
					// stamp + begin 时间转换
					if (StringUtils.isNotEmpty(qbaoLog.getStamp()) && StringUtils.isNotEmpty(qbaoLog.getBegin())) {
						try {
