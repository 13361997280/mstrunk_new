...
public class EsDataServiceSql {
	...
	public static void main(String[] args) throws Exception, IOException {
		...
		String sql = Instance.getInstance().sql_Select;

		ArrayList<EsDictPo> results = Instance.getInstance().select(sql);
		for (int i = 0; i < results.size() && i < 100; i++) {
			System.out.println(results.get(i).toString());
		}
		System.out.println("ok!  count = " + results.size());

		// System.out.prin
		// tln("==================   count  ======================");
		// int count =
		// Instance.getInstance().count(Instance.getInstance().sql_Count);
		// System.out.println("count = " + count);
		//

		// System.out.println("==================  selectIds  ======================");
		// ArrayList<String> selectIds =
		// Instance.getInstance().selectIds(Instance.getInstance().sql_SelectIds);
		//
		// for (int i = 0; i < selectIds.size() && i < 100; i++) {
		// System.out.println("==" + selectIds.get(i));
		// }
	}
