...
/**
 * 由于qbaolog独立es库，所以单独设置一个数据源
 * @author fanyunlong
 *
 */
public class QbaoLogInputService {

	private static ESLogger logger = Loggers.getLogger(PoiEsDataService.class);
	private static QbaoLogInputService dataInputEsService;
	private static TransportClient client;

	public static final QbaoLogInputService getInstance() {
		try {
			if (dataInputEsService == null) {
				synchronized (PoiEsDataService.class) {
					// -----------------es集群连接--------------------
					Settings settings = Settings.builder().put("cluster.name", Config.get().get("es.qb.cluster.name")) // 设置集群名
					...
					client = new PreBuiltTransportClient(settings)
							.addTransportAddress(new InetSocketTransportAddress(new InetSocketAddress(
									Config.get().get("es.qb.host1"), Config.get().getInt("es.qb.port", 9300))))
							.addTransportAddress(new InetSocketTransportAddress(new InetSocketAddress(
									Config.get().get("es.qb.host2"), Config.get().getInt("es.qb.port", 9300))))
							.addTransportAddress(new InetSocketTransportAddress(new InetSocketAddress(
									Config.get().get("es.qb.host3"), Config.get().getInt("es.qb.port", 9300))));
					dataInputEsService = new QbaoLogInputService();
				}
			}
		} catch (Exception ex) {
			ex.printStackTrace();
	 ...
	 */
	public String dealQbaoLogDataInput() {
		...
		return getBatchDataByScroll(lstLong, System.currentTimeMillis(), LoadValues.LABEL_QBAOLOG, LoadValues.LABEL_QBAOLOG, "", "stamp");
	}
	 ...
	 * 
	 */
	public String getBatchDataByScroll(Object fromTime, Object endTime, String indexName, String type,
			String fields, String queryIndex) {
		Long totalNum = 0L;
		try {
			BoolQueryBuilder qb = QueryBuilders.boolQuery();
			...
			searchSourceBuilder.postFilter(qb);
			SearchResponse response = searchRequestBuilder.setSource(searchSourceBuilder).setSize(10000).setScroll(new TimeValue(600000))
	...
	public String addRecordTask(String type, Long totalNum, Date inputTime) throws Exception {
		...
		try {
			...
			BulkRequestBuilder bulkRequest = QbaoLogInputService.client.prepareBulk();
			bulkRequest.add(
	...
	public static void main(String[] args) throws Exception {
		 ...
		 * 
		 */
		QbaoLogInputService service = QbaoLogInputService.getInstance();
